...
test('H3 foundations keep audio suppression local and gate unverified post-processing workflows', () => {
  ...
  assert.match(profiles, /minimax_h3_text_to_video/)
  assert.match(profiles, /source: null, checksum: null/)
  assert.match(upscale, /RTX_VSR_SETUP_REQUIRED/)
  assert.match(upscale, /nvidia-vfx/)
  assert.match(home, /Upscale Video/)
  assert.match(home, /The original render will never be replaced/)
})
