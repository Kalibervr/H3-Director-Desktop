  ...
  return <section className="min-w-0 border-t border-white/10 bg-[#090c11] px-6 py-3">
    ...
    <div className="flex h-[96px] gap-3 overflow-x-auto pb-1">
      {project?.scenes.map((scene, index) => {
        ...
        const percent = scene.progress_value !== null && scene.progress_max
          ? Math.round((scene.progress_value / scene.progress_max) * 100) : null
        return <div key={scene.id} className={`group relative flex w-64 shrink-0 items-center gap-3 rounded-xl border p-2.5 ${selected ? 'border-amber-300/40 bg-amber-300/[0.06]' : 'border-white/10 bg-white/[0.02]'}`}>
          <button onClick={() => onSelect(scene)} className="flex min-w-0 flex-1 items-center gap-3 text-left"><div className="flex h-14 w-20 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-black">{media ? media.video ? <video src={media.url} muted preload="metadata" className="h-full w-full object-cover" /> : <img src={media.url} alt={`${scene.name} reference`} className="h-full w-full object-cover" /> : <Film className="h-5 w-5 text-zinc-700" />}</div><div className="min-w-0 flex-1"><div className="text-[9px] uppercase tracking-wider text-zinc-600">Scene {String(index + 1).padStart(2, '0')}</div><div className="truncate text-xs font-semibold">{scene.name}</div><div className="mt-1 truncate text-[9px] text-zinc-600">{scene.duration_seconds}s · {scene.current_phase ?? statusLabels[scene.status]}{percent !== null ? ` · ${percent}%` : ''}</div>{percent !== null && <div className="mt-1 h-1 overflow-hidden rounded-full bg-white/5"><div className="h-full bg-amber-300" style={{ width: `${percent}%` }} /></div>}</div></button>
