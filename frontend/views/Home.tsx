  ...
  return <div className="h3-director-ui h-screen overflow-hidden bg-[#07090d] text-zinc-100">
    <div className="grid h-full grid-cols-[250px_minmax(0,1fr)_360px] grid-rows-[minmax(0,1fr)_150px]">
      ...
      <aside className="row-span-2 min-h-0 overflow-y-auto border-l border-white/10 bg-[#0b0e13] p-5">
        ...
        {project && <div className="mt-4 rounded-xl border border-white/10 bg-white/[0.025] p-3"><div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-zinc-500">Model</div><H3ThemedSelect value={project.workflow_profile_id} ariaLabel="Model profile" onChange={profileId => { const profile = workflowProfileRegistry.get(profileId as H3WorkflowProfileId); const mode = profile?.modes[0]?.id as H3WorkflowMode | undefined; if (!mode) return; void updateH3Project(project.id, { workflow_profile_id: profileId as H3WorkflowProfileId, workflow_mode: mode }).then(async next => { if (profileId === 'ltx_2_5_image_to_video' && scene) return updateH3Scene(next.id, scene.id, { aspect_ratio: '16:9 (Widescreen)', resolution_megapixels: 0.9, width: 1280, height: 704, fps: 24, frame_count: 121, ltx_prompt_enhance: true }); return next }).then(replaceProject).catch(error => setWorkspaceError(error instanceof Error ? error.message : 'Model profile could not be saved.')) }} options={workflowProfileRegistry.list().filter(profile => profile.status === 'verified' || profile.status === 'runtime_verified').map(profile => ({ value: profile.id, label: `${profile.displayName} · ${profile.label}` }))} /><div className="mt-3 text-[10px] font-semibold uppercase tracking-[0.16em] text-zinc-500">Mode</div><H3ThemedSelect value={project.workflow_mode} ariaLabel="Workflow mode" onChange={mode => void updateH3Project(project.id, { workflow_mode: mode as H3WorkflowMode }).then(replaceProject).catch(error => setWorkspaceError(error instanceof Error ? error.message : 'Workflow mode could not be saved.'))} options={(selectedWorkflowProfile?.modes ?? []).filter(mode => mode.verified).map(mode => ({ value: mode.id, label: mode.label }))} />{selectedWorkflowMode && <p className="mt-2 text-[10px] text-zinc-500">{selectedWorkflowMode.capabilities.imageToVideo ? 'Reference image required for new shots.' : 'This profile mode is not verified.'} {selectedWorkflowProfile?.status === 'runtime_verified' ? 'Local template is verified; H3 render binding is still pending.' : 'Advanced workflow details remain hidden.'}</p>}</div>}
        ...
        <div className="mt-3 grid grid-cols-2 gap-2"><button onClick={() => void startSequence('from_here')} disabled={!project || !scene || selectedWorkflowProfile?.status !== 'verified' || runtimeStatus !== 'connected' || Boolean(activeRun) || sequenceStarting} className="rounded-lg border border-amber-300/20 px-3 py-2.5 text-xs text-amber-200 disabled:opacity-30">Render From Here</button><button onClick={() => void startSequence('all')} disabled={!project || selectedWorkflowProfile?.status !== 'verified' || runtimeStatus !== 'connected' || Boolean(activeRun) || sequenceStarting} className="rounded-lg border border-amber-300/20 px-3 py-2.5 text-xs text-amber-200 disabled:opacity-30">Render All</button></div>
