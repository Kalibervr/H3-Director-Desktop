...
def test_render_version_promotes_final_prompt_provenance_from_immutable_metadata(tmp_path: Path) -> None:
    store = H3ProjectStore(tmp_path / "Projects")
    project = store.create_project("Prompt provenance")
    version = _version(Path(project.project_root) / "scenes" / "scene_001" / "renders", 1)
    Path(version.metadata_file).write_text(json.dumps({
        "raw_user_prompt": "raw scene prompt", "final_submitted_prompt": "exact submitted prompt",
        "native_prompt_enhance": True, "render_elapsed_seconds": 12.5,
    }), encoding="utf-8")
    saved = store.add_render_version(project.id, project.scenes[0].id, version)
    rendered = saved.scenes[0].render_versions[0]
    assert rendered.raw_user_prompt == "raw scene prompt"
    assert rendered.final_submitted_prompt == "exact submitted prompt"
    assert rendered.native_prompt_enhance is True
    assert rendered.render_elapsed_seconds == 12.5
